...
function Post(id,content,type,time,title,biaoqian,tuijian,liuyanCount){
    this.id = id;
...
Post.getArticle = function(callback) {
	...
	mysql.query(sql, function(err, rs, fields){
			...
			//id,content,type,time,title,biaoqian,tuijian
				...
				console.log(doc+"======================================");
					var article=new Post(doc.id,doc.content,doc.type,doc.time,doc.title,doc.biaoqian,doc.tuijian,doc.liuyanCount)
					array.push(article);
				});	
				callback(null,array);
			}
...
Post.findArticle = function(titleSearch,callback) {//返回所有文章
  var sql = "select a.*,(SELECT count(*) from liuyan l WHERE l.artical_id=a.id) as liuyanCount,t.`name` as type "+
	...
	mysql.query(sql, function(err, rs, fields){
			...
			//id,content,type,time,title,biaoqian,tuijian
				rs.forEach(function(doc){
					var post=new Post(doc.id,doc.content,doc.type,doc.time,doc.title,doc.biaoqian,doc.tuijian,doc.liuyanCount)
					array.push(post);	
				});
				callback(null,array);
			}
